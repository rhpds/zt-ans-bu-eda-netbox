= Module 7: Event-Driven Ansible and NetBox in Action
:icons: font
...
*Step 4:* Some other data points we had to pre-create were *Custom Fields* for the `ansible_host` and `ansible_port` variables. You can review these in the *Customization > Custom Fields* menu of the sidebar in NetBox.
...
*Step 5:* Another setting we set in advance are the *Config Contexts* for the custom data we want to store for the `NTP Servers` and `Login Banner`. You can review these in the *Provisioning > Config Contexts* menu of the sidebar in NetBox.
...
*Step 1:* In the NetBox tab, go to the left sidebar and navigate to *Operations > Webhooks*
...
Now we can go to *Operations > Event Rules* to create the three events we want to forward.
...
** *AAP* tab → Automation Execution > Infrastructure > Inventories → *NetBox Dynamic Inventory* → *Hosts* → `cat1`
====
...
*Step 2:* A quick way to see if an event was triggered is to check the *Fire count* counter in the *Rulebook Activations* screen within *Automation Decisions > Rulebook Activations*.
...
*Step 3:* For a more detailed output, navigate to *Automation Decisions > Rule Audit*. There you will see which Job Template was triggered.
...
*Step 6:* If you want to see the actual Job Template output, navigate to *Automation Execution > Jobs* and check the Job queue to see the details.
...
*Step 2:* Navigate to *Devices > Devices* in the left sidebar.
...
*Step 3:* Switch to the *NetBox Terminal* tab.
...
For the Job Templates to be pre-created in Module 5, the `NetBox Dynamic Inventory` must exist.

*If the inventory doesn't exist:*

*Step 1:* First, go to Module 2 and create the `NetBox Dynamic Inventory`.
...
For the Dynamic Inventory in Module 2 to work, we need some NetBox pre-loaded content.
...
*Step 1:* Go to the *NetBox Terminal* tab.
